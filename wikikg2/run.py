...
def main(args):
    ...
    args.save_path = 'log/%s/%s/%s-%s-%s/%s'%(args.dataset, args.model, args.hidden_dim, args.gamma,args.learning_rate, time.time()) if args.save_path == None else args.save_path
    ...
    total = sum([param.nelement() for param in kge_model.parameters()])
